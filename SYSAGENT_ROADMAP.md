...
- Improved desktop automation feedback: queue-resume requests now include the fresh post-command screenshot in AI Engine screen summarization, so multi-step GUI flows can reason from the latest visible state before proposing the next approved action.
- Added the first post-command visual verification loop: Spring exposes owner-scoped task verification, AI Engine evaluates node output plus the fresh screen summary, and Angular displays verified/failed/uncertain feedback before continuing queued remote steps.
- Added verification repair routing: failed/uncertain desktop action verification now stops queue auto-resume, asks the AI Engine for one corrected approved action using the latest screen summary, and keeps raw screenshot context available for that repair request.
- Hardened node installability with package metadata, `sysagent-node doctor` diagnostics, backend/token readiness checks, GUI helper checks, and config validation before background service installation.
- Added node bootstrap and GUI safety hardening: device registration tokens now produce a `sysagent-node bootstrap ... --install-service` command, Windows GUI click/type scripts fail instead of targeting the wrong window when focus cannot be verified, and verification repair is capped at one automatic proposal per step.
- Added GitHub/deployment safety hardening: secret/config/runtime artifacts are ignored, Hibernate SQL logging is off by default, AI Engine calls can require `X-SysAgent-AI-Key`, and production startup refuses a remote AI Engine without that shared secret.
- Clarified AI language routing safety: deterministic intent/planner shortcuts are latency hints only, not app-specific behavior, and unsupported-language input must fall back to the LLM/Agent Hub route instead of being forced through hardcoded rules.
