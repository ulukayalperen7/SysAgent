...
export class AgentService {
    ...
    getAgentProfiles(): Observable<AgentProfile[]> {
        return this.http.get<ApiResponse<AgentProfile[]>>(`${this.apiUrl}/profiles`).pipe(
            map(response => {
                if (response.status !== 'SUCCESS' || !response.data) {
                    throw new Error(response.message || 'Agent profiles response did not include usable data.');
                }
                return response.data;
            }),
            catchError(error => {
                console.error('AgentService: Error loading agent profiles', error);
                throw error;
            })
        );
    }
